...
function AddSheet({ open, onClose, onPick }) {
  const DS = window.BudgetSplitDesignSystem_f6e8de;
  const { Icon } = DS;
  const actions = [
    { key: 'expense', label: 'Expense', icon: 'minus-circle', tint: 'var(--expense)', desc: 'Record spending' },
    { key: 'itemized', label: 'Split a bill', icon: 'users', tint: 'var(--accent)', desc: 'Split line items between people' },
    { key: 'settle', label: 'Settle up', icon: 'check-circle', tint: 'var(--settle)', desc: 'Record a payment' },
    { key: 'income', label: 'Income', icon: 'plus-circle', tint: 'var(--income)', desc: 'Money you received' },
  ];
  return (
    <div
      onClick={onClose}
    ...
    >
      ...
      >
        ...
        {actions.map((a) => (
          <div key={a.key} onClick={() => { onClose && onClose(); onPick && onPick(a.key); }} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '10px 4px', minHeight: 56, cursor: 'pointer' }}>
