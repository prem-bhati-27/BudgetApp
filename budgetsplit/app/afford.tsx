...
type Verdict = 'comfortable' | 'tight' | 'no';
...
export default function AffordScreen() {
  ...
  useEffect(() => {
    (async () => { try { setCash((await getCashPosition(db)).available); } catch { setCash(0); } })();
  }, []);
  ...
  const verdict: Verdict = remaining < 0 ? 'no' : remaining >= Math.max(available * 0.2, 0) ? 'comfortable' : 'tight';
  const V = {
    comfortable: { color: colors.income, icon: 'check-circle' as const, title: 'Yes — you can afford it', sub: `Leaves ${formatCompact(remaining)} of spendable cash.` },
    tight:       { color: colors.healthAmber, icon: 'alert-circle' as const, title: 'Yes, but it’s tight', sub: `Only ${formatCompact(remaining)} would be left. Maybe wait or save up.` },
    no:          { color: colors.expense, icon: 'x-circle' as const, title: 'Not right now', sub: `You’d be short by ${formatCompact(-remaining)}. Consider saving toward it instead.` },
  }[verdict];
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.cashRow}>
            <Text style={styles.cashLabel}>Spendable cash right now</Text>
...
const styles = StyleSheet.create({
  ...
  cashRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.bgCard, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, padding: space.md, ...shadow.sm },
